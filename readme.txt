Minute to Midnight is a dead man's switch set for 23:59.
client.py is a  simple client script that is ran from the command line and sends data to listener.py. The port number and IP address of the listener are to be passed in as arguments along with the data(passphrase) to send.
...
dms_v3.py requires the absolute path of the target file to be cleared in case of failed authentication(unless said file is in the same directory as dms_v3.py) and the passphrase to authenticate against to be passed in as arguments.
All scripts contain a help option (-h / --help).
dms_v3.py and listener.py need to be in the same directory
